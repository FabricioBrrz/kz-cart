...
local function GeneratePlate()
    local plate = "CA" .. tostring(QRCore.Shared.RandomInt(2) .. QRCore.Shared.RandomStr(2) .. QRCore.Shared.RandomInt(1) .. QRCore.Shared.RandomStr(1))
...
QRCore.Commands.Add('rcart', 'Reset Cart (Admin Only)', {{ name = 'identifiant', help = 'Placa (ID) da carroça' }}, true, function(args)
...
QRCore.Functions.CreateCallback('kz-cart:server:GetUserCart', function(source, cb, currentStable)
    local src = source
...
QRCore.Functions.CreateCallback('kz-cart:server:GetUserCartAll', function(source, cb)
    local src = source
    local Player = QRCore.Functions.GetPlayer(src)
	local Playercid = Player.PlayerData.citizenid
...
QRCore.Functions.CreateCallback('kz-cart:server:CheckVehicleOwner', function(source, cb, tempplate)
    local src = source
    local Player = QRCore.Functions.GetPlayer(src)
	local Playercid = Player.PlayerData.citizenid
...
QRCore.Functions.CreateCallback('kz-cart:server:CheckPlateInventory', function(source, cb, plate)
