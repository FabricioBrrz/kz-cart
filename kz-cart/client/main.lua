...
RegisterNetEvent("kz-cart:client:CartList", function()
    QRCore.Functions.TriggerCallback('kz-cart:server:GetUserCart', function(result)
        if result == nil then
            ...
            exports['qr-menu']:openMenu(MenuPublicStableNocart)
        else
            ...
            exports['qr-menu']:openMenu(MenuPublicStableOptions)
        end
    end, currentStable)
end)
...
RegisterNetEvent("kz-cart:client:CartListSell", function()
    QRCore.Functions.TriggerCallback('kz-cart:server:GetUserCartAll', function(result)
        if result == nil then
            ...
            exports['qr-menu']:openMenu(MenuCartSellNo)
        else
            local MenuCartSell = {
                {
                    header = "| "..Lang:t('menu.header_glob').." |",
            ...
            exports['qr-menu']:openMenu(MenuCartSell)
        end
    end)
end)
...
RegisterNetEvent("kz-cart:client:CartListGive", function()
    QRCore.Functions.TriggerCallback('kz-cart:server:GetUserCartAll', function(result)
        if result == nil then
            ...
            exports['qr-menu']:openMenu(MenuCartGiveNo)
        else
            local MenuCartGive = {
                {
                    header = "| "..Lang:t('menu.header_glob').." |",
            ...
            exports['qr-menu']:openMenu(MenuCartGive)
        end
    end)
end)
...
RegisterNetEvent('kz-cart:client:TakeOutPublic', function(vehicle)
    if vehicle.state == Lang:t('state._in') then
                ...
                model = vehicle.model
                plate = vehicle.plate
                exports['qr-core']:Notify(9, Lang:t('state.check'), 2000, 0, 'toast_awards_set_q', 'awards_set_q_011', 'COLOR_WHITE')
                Wait(1000)
                exports['qr-core']:SpawnVehicle(model, function(veh)
                    SetEntityHeading(veh, StableCart[currentStable].spawnPoint.w)
                    TaskWarpPedIntoVehicle(PlayerPedId(), veh, -1)
                    Citizen.InvokeNative(0x400F9556, veh, plate)
                    SetEntityAsMissionEntity(veh, true, true)
                    closeMenuFull()
                    local tempplate = exports['qr-core']:GetPlate(veh)
    ...
    elseif vehicle.state == Lang:t('state._out') then
        exports['qr-core']:Notify(9, Lang:t('state.out'), 2000, 0, 'mp_lobby_textures', 'cross', 'COLOR_WHITE')
    elseif vehicle.state == Lang:t('state._ss') then
        exports['qr-core']:Notify(9, Lang:t('state.ssheriff'), 2000, 0, 'mp_lobby_textures', 'cross', 'COLOR_WHITE')
    end
end)
...
RegisterNetEvent('kz-cart:client:PutInStable', function()
    for k, v in pairs(StableCart) do
        ...
        if PutInDist <= 10 then
            ...
            local cart = QRCore.Functions.GetClosestVehicle()
            local tempplate = exports['qr-core']:GetPlate(cart)
            if (tempplate ~= nil) then
                QRCore.Functions.TriggerCallback('kz-cart:server:CheckVehicleOwner', function(owned)
                    if owned then
                        TriggerServerEvent('kz-cart:server:UpdateStatePut', 1, k, tempplate)
                        Wait(500)
                        QRCore.Functions.DeleteVehicle(cart)
                        exports['qr-core']:Notify(9, Lang:t('success.putin'), 2000, 0, 'mp_lobby_textures', 'cross', 'COLOR_WHITE')
                    else
                        exports['qr-core']:Notify(9, Lang:t('error.putinno'), 2000, 0, 'mp_lobby_textures', 'cross', 'COLOR_WHITE')
                    end
                end, tempplate)
            else
                exports['qr-core']:Notify(9, Lang:t('error.putinerr'), 2000, 0, 'mp_lobby_textures', 'cross', 'COLOR_WHITE')
            end
        end
    end
end)
...
RegisterNetEvent('kz-cart:client:CartInventory', function()
    ...
    QRCore.Functions.TriggerCallback('kz-cart:server:CheckPlateInventory', function(result) 
        if result == nil then
            exports['qr-core']:Notify(9, Lang:t('error.inverr'), 2000, 0, 'mp_lobby_textures', 'cross', 'COLOR_WHITE')
        else
            for k, v in pairs(result) do
...
CreateThread(function()
    for k, v in pairs(StableCart) do
        ...
        exports['qr-core']:createPrompt(StableCart[k].name, StableCart[k].takeCart, 0xF3830D8E, Lang:t('other.prompt'), {
            type = 'client',
...
RegisterCommand('debug_iccart', function()
    local plate = exports['qr-core']:GetPlate(GetVehiclePedIsIn(PlayerPedId()))
    print(plate)
end)
...
exports['qr-target']:AddTargetModel({-1698498246, -2044900246, 1761016051}, {
	options = {
		{
